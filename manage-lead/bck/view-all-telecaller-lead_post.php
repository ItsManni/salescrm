...
$FinalStatus = $IMSSetting->GetFinalTelecallerLeadStatus();
$filter_final_status = "";
if($FinalStatus != "")
{
...
if($UserType == "Center Manager" || $UserType == "Lead Counsellor" || $UserType == "Counsellor")
{
...
/*if($UserType == "Telecaller")
{
    $filter = $filter.$filter_final_status;
}*/
// echo $filter;
$totalRecordwithFilter = $core->_getTotalRows($conn,'telecaller_leads', $filter);
## Total number of record with filtering
$totalRecords = $totalRecordwithFilter;
$filter = $filter." ORDER BY ".$columnName." ".$columnSortOrder;
$filter = $filter." limit ".$row.",".$rowperpage;
$all_lead_details = $core->_getTableRecords($conn,'telecaller_leads', $filter);
...
$where = " where 1";
$lead_status_temp = $core->_getTableRecords($conn,'telecaller_lead_status',$where);
...
foreach ($all_lead_details as $all_lead_details_value)
{
  ...
  if($UserType == "Counsellor" || $UserType == "Lead Counsellor")
  {
    $ViewDetails_html = "<a href='view-telecaller-lead-details?LeadID=".$Lead_ID_encrypted."'><span class='badge bg-info badge-sm  me-1 mb-1 mt-1'>View Details</span></a><br>";
  }
  else
  {
    $ViewDetails_html = "<a href='view-telecaller-lead-details?LeadID=".$Lead_ID_encrypted."'><span class='badge bg-info badge-sm  me-1 mb-1 mt-1'>View Details</span></a>";
    if($Status != $FinalStatus)
    {
         $ViewDetails_html =  $ViewDetails_html."<br><span style='cursor: pointer;' class='badge bg-primary' onclick='OpenModal_QuickChangeTelecallerLeadStatus($ID,\"$Status\")'>Change Status</span>";
    }
  }
  if($UserType == "Counsellor" || $UserType == "Lead Counsellor")
  {
    if($Status != $FinalStatus)
    {
        $Action_html = "<span style='cursor: pointer;' class='badge bg-warning' onclick='AssignTelecallerLeadtoCounsellor($ID)'>Assign to me</span>";
    }
    else
    {
        $Action_html = "N.A.";
  ...
  {
     $Action_html = "<a class='btn text-danger btn-sm' data-bs-toggle='tooltip'  onclick='EditTelecallerLead_modal($ID)' data-bs-original-title='Edit'><span class='fe fe-edit fs-14'></span></a>";
  }
